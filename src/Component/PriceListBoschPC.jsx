...
const PriceListBoschPC = () => {
  const { allData, modifiedOn, loading } = useContext(boschPricePC);
  ...
  // no search → sirf 100 items
  if (!q) {
    return allData.slice(0, 100);
  }

  // search → safe filtering
  ...
  return (
    <div className="px-4 pb-12 pt-6 flex flex-col items-center gap-6">
      ...
      <div className="w-full max-w-4xl bg-white rounded-2xl px-6 py-5 border border-slate-200">
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 rounded-xl bg-slate-50 border border-slate-200 focus-within:ring-2 focus-within:ring-blue-600/10">
      ...
      <div className="w-full max-w-4xl flex flex-col gap-3">
        {visibleData.map((item) => (
          <Link
            ...
            className="group"
          >
            <div className="bg-white rounded-2xl px-5 py-4 border border-slate-200 flex items-center justify-between gap-4 hover:shadow-md transition">
              ...
              <span className="text-slate-300 group-hover:text-slate-700 transition text-xl shrink-0">
                →
              </span>
            </div>
          </Link>
